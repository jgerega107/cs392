...
void *sum_array(void *ptr) {
    thread_args* tapr = (thread_args*)ptr;
    int partial_sum = 0;
    for(int i = tapr->start_index; i < tapr->length; i+=2){
        partial_sum+=array[i];
    }
    ...
    if(pthread_mutex_unlock(&lock) != 0){
        fprintf(stderr, "Warning: Cannot umlock mutex. %s.\n", strerror(errno));
    }
    pthread_exit(NULL);
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 2; i++){
        ...
        if (pthread_create(&threads[i], NULL, sum_array, &targs[i]) != 0){
